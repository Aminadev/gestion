                  ...
                  if(isset($_POST['btnEnregister']))
                    {
                      ...
                      if(empty($nom) || empty($prenom) || empty($adresse) || empty($datenaiss) || empty($email) || empty($pwd1) || empty($tel) || empty($sexe)){
                             ...
                             echo "<script>alert('VEUILLEZ RENSEIGNEZ DES VALEURS DE CHAMPS VALIDE')</script>";
                              ...
                              if(!$secretaires->DoublonsSecretaire(trim($nom),trim($prenom),trim($tel),trim($email))){                                
                                    $DonneMedecin = [
                                    'nom'=>$nom,
                                    'prenom'=>$prenom,
                                    'adresse'=>$adresse,
                                    ...
                                    'idserv'=>$service
                                        ...
                                        $verif = $secretaires->InsertMedecin($DonneMedecin);
                                        if($verif){
                                        header('location:listeSecretaire.php');
                                        }
                                      }else{
                                        echo "<script>alert('DESOLE LE SECRETAIRE QUE VOUS VOULEZ AJOUTER EXISTE DEJA')</script>";
                                      }
                                }
  ...
  <div class="group">      
    ...
    <div class="group">
        <select name="service" class="inputMaterial">
            <option value="">Choisi une Service  </option>
            <?php
                $service = new Service();
                $Ser = $service->select();
                //var_dump($med);
