          	...
          	echo "<script>alert('VEUILLEZ RENSEIGNE VOTRE LOGIN SVP');</script>";   
			}elseif(empty($password))
               {
                  echo "<script>alert('VEUILLEZ RENSEIGNE VOTRE MOT DE PASSE SVP');</script>";   
                        ...
                        $requete=$base->query("SELECT * FROM utilisateur WHERE profil='etu' ");
						if($requete){
							while($result=$requete->fetch())
							{
								if($result['login']==$login && $result['pwd']==$password && $result['profil']=="etu")
								{
							...
							echo "<script>alert('DESOLE IL FAUT ETRE ETUDIANT POUR EVALUER UN FORMATEUR !');</script>";   
