          	...
          	echo "<script>alert('VEUILLEZ RENSEIGNE VOTRE LOGIN SVP');</script>";   
			}elseif(empty($password))
               {
                  echo "<script>alert('VEUILLEZ RENSEIGNE VOTRE MOT DE PASSE SVP');</script>";   
                        ...
                        $requete=$base->query("SELECT * FROM utilisateur WHERE profil='admin' ");
						if($requete){
							while($result=$requete->fetch())
							{
								if($result['login']==$login && $result['pwd']==$password && $result['profil']=="admin")
								{
							...
							echo "<script>alert('DESOLE CETTE PARTIE EST RESERVE AUX MEMBRES DE L ADMINISTRATION!');</script>";   
