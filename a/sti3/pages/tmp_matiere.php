<?php 
  ...
  if(isset($_REQUEST['logout']))
  {
    ...
    $date=DATE('Y-m-d');
      ...
      {
        if (empty($formateur)) {
          echo "<script>alert('CHOISISSEZ UN FORMATEUR ET REESSAYER !! ');</script>";
        }else{
            $courok=$conn->exec("UPDATE cours SET iduser='$formateur' WHERE idcours='$idmod' ");
            if($courok)
            {
                  $req=$conn->exec("INSERT INTO ligne_prof_cours(idprof,idcours,dateLignPrCrs) VALUES('$formateur','$idmod','$date') ");
                  if ($req) {              
                      echo "<script>alert('VALIDATION D UN NOUVEAU MODULE ')</script>";
                      echo"<script>window.location.href='matiere_prof.php';</script>";
                }else{
                    echo "<script>alert('VALIDATION NON REUSSIT 1')</script>";
                    //echo"<script>window.location.href='matiere_prof.php';</script>";
                }
            }else{
                  echo "<script>alert('VALIDATION NON REUSSIT 2')</script>";
              }
...
<form method="POST" action="">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <center>
          <h1 class="h3 mb-4 text-gray-800">veuillez choisir un formateur pour le nouveau module <b> <?=$mod?></b></h1>

              <?php   $allprof=AllFormateur();
               ...
               ?>
                        ...
                        <?php 
                          if($allprof)
                          {
                              while($pr=$allprof->fetch())
                              { //$ligne=finduserByMatricule($pr['iduser']);
                              ?>
                              <option value="<?=$pr['iduser']?>"><?php echo $pr['prenom']." ".$pr['nom'];?> </option>
                              <?php 
                              }
                          }
                         ?>
                          ...
                          <table  width="40%">
                            <tr><td><input class="btn btn-primary btn-user btn-block" type="submit"  name="BtnValider"></td>
                            </tr>
                          </table>
                  </center>
